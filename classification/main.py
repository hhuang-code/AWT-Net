...
def backup(opts):
    if not opts.eval:
        os.system('cp config.py ' + opts.exp_name + '/' + 'config.py.backup')
        ...
        os.system('cp modelnet.py ' + opts.exp_name + '/' + 'modelnet.py.backup')
...
def train(gpu, opts):
    ...
    train_set = ModelNet40(opts.data_path, opts.n_pts, 'train')
    ...
    test_set = ModelNet40(opts.data_path, opts.n_pts, 'test')
    ...
    for epoch in range(opts.epochs):
        ...
        for i, (data, label, name) in enumerate(train_loader):
...
def test(gpu, opts):
    ...
    for name, acc in zip(shape_names, per_class_acc):
        msg = '{}: {:.6f}'.format(name, acc)
        print(msg)
        logging.info(msg)
